...
export const CHARGING_DURATION_MS = 120000;
export function chargingPercent(start: Date, now = Date.now()) {
  return Math.min(100, Math.max(0, Math.floor((now - start.getTime()) / CHARGING_DURATION_MS * 100)));
}
...
export class ChargingService implements OnModuleInit, OnModuleDestroy {
  ...
  async start(id: number, userId: number) {
    const booking = await this.db.transaction(async manager => {
      ...
      if (item.status !== 'confirmed') throw new ConflictException('Payment must be confirmed before charging');
  ...
  async stop(id: number, userId: number) {
    const result = await this.db.transaction(async manager => {
      ...
      if (!item) throw new NotFoundException('Booking not found');
      if (!item.chargingStartedAt) throw new ConflictException('Charging has not started');
      if (item.chargingCompletedAt || item.chargingStoppedAt) return { item, notice: null };
      const now = new Date();
      ...
      const notice = await manager.save(UserNotification, manager.create(UserNotification, {
        user: { id: userId }, bookingId: item.id,
        title: percentage === 100 ? 'Charging complete' : 'Charging stopped',
        message: 'Charging for slot ' + item.slotNumber + (percentage === 100 ? ' completed at 100%.' : ' stopped at ' + percentage + '%.'),
      }));
      return { item, notice };
    });
  ...
  private view(item: BookingEntity) {
    return {
      bookingId: item.id, slotNumber: item.slotNumber, simulated: true,
      startedAt: item.chargingStartedAt, completedAt: item.chargingCompletedAt,
      stoppedAt: item.chargingStoppedAt,
      status: item.chargingCompletedAt ? 'completed' : item.chargingStoppedAt ? 'stopped' : 'charging',
      percentage: item.chargingCompletedAt ? 100 : chargingPercent(item.chargingStartedAt!, item.chargingStoppedAt?.getTime() ?? Date.now()),
  ...
  async completeDue() {
    ...
    try {
      const candidates = await this.db.getRepository(BookingEntity).find({
        where: { chargingStartedAt: Not(IsNull()), chargingCompletedAt: IsNull(), status: 'confirmed' },
      ...
      for (const candidate of candidates) {
        ...
        const result = await this.db.transaction(async manager => {
          const item = await manager.findOne(BookingEntity, {
            where: { id: candidate.id }, relations: { user: true },
            lock: { mode: 'pessimistic_write', tables: ['bookings'] },
          });
          if (!item || item.chargingCompletedAt || item.status !== 'confirmed') return null;
          item.chargingCompletedAt = new Date();
          item.status = 'completed';
          await manager.save(item);
          const notice = await manager.save(UserNotification, manager.create(UserNotification, {
            user: { id: item.user.id }, bookingId: item.id,
            title: 'Charging complete',
            message: 'Simulated charging for slot ' + item.slotNumber + ' reached 100%. Your session is complete.',
          }));
          return { notice, userId: item.user.id };
        });
        if (result) await this.notifications.deliver(result.userId, result.notice);
      }
    } catch { this.logger.warn('Charging update failed; retrying on the next interval'); }
