...
export class AdminController {
  ...
  @Patch(':id/activate')                                   // PATCH /admin/1/activate?value=false
  setActive(@Param('id', ParseIntPipe) id: number, @Query('value') value: string) {
    return this.adminService.setActive(id, value === 'true');
  }

  @Delete(':id')                                           // DELETE /admin/1
  remove(@Param('id', ParseIntPipe) id: number) {
    return this.adminService.remove(id);
  }

  // ---- USER MANAGEMENT (admin manages hosts/drivers/technicians) [JWT] ----
  @Post(':id/user')                                        // POST /admin/1/user  (add a user to manage)
  @UseGuards(JwtAuthGuard)
  addUser(@Param('id', ParseIntPipe) id: number, @Body() dto: CreateManagedUserDto) {
    return this.adminService.addUser(id, dto);
  }

  @Get(':id/users')                                        // GET /admin/1/users?role=host
  getUsers(@Param('id', ParseIntPipe) id: number, @Query('role') role?: string) {
  ...
  @UseGuards(JwtAuthGuard)
  setUserStatus(@Param('userId', ParseIntPipe) userId: number, @Query('value') value: string) {
  ...
  @UseGuards(JwtAuthGuard)
  createDispute(@Param('id', ParseIntPipe) id: number, @Body() dto: CreateDisputeDto) {
